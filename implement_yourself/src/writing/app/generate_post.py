...
This is the single source of truth for the generate -> [review -> edit] x N
...
async def generate_post(
    ...
    for i in range(settings.num_reviews):
        iteration = i + 1
        logger.info("Review/edit iteration %d/%d...", iteration, settings.num_reviews)

        reviews_result = await review_post(post, guideline, profiles)
        all_reviews.append(reviews_result)

        if not reviews_result.reviews:
            logger.info("Iteration %d: No issues found. Skipping edit.", iteration)
            continue

        logger.info(
            "Iteration %d: %d review(s) found.", iteration, len(reviews_result.reviews)
        )
