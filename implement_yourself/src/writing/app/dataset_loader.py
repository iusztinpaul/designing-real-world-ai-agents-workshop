...
def load_dataset() -> list[DatasetEntry]:
    ...
    Returns:
        List of DatasetEntry objects. Returns [] if dataset is missing (with warning).
    """
    index_path = DATASET_DIR / "index.yaml"
    if not index_path.exists():
        logger.warning("Dataset index not found: %s", index_path)
        return []
...
def load_examples() -> DatasetExamples:
    ...
    Returns:
        DatasetExamples with post and media examples.
        Returns empty DatasetExamples if dataset directory is missing.
    """
