...
- **`deep_research`** — "Research a topic using Gemini with Google Search grounding." Args: `working_dir`, `query`.
...
- DO NOT add the `@opik.track(...)` decorator yet — that lands in #020.
...
1. The attendee makes sure `.env` has `GOOGLE_API_KEY` populated (still required by `Settings`, even though no Gemini call is made yet — Pydantic validates at import time).
